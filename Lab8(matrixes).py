...
def get_matrix_dimensions():
     '''(none) --> int, int
        returns dimensions for the matrix
     '''
     got_it_flag=False
...
def sum_above_diagonal(m):
     ...
     for row_index in range(len(m)):
          for column_index in range(len(m)):
               if row_index<=column_index:
                    sum += m[row_index][column_index]
     return sum
...
def index_of_max_sum_row(m):
     ...
     index = 0
     summation = sum(m[0])
     for i in range(len(m)): #i will be the index
        newsum = sum(m[i])
        if newsum > summation:
             summation = newsum
             index = i
...
a=dimensions[0]
b=dimensions[1]
thematrix = (make_matrix_random(a, b, 1, 10))
#print(thematrix)
#print(sum_above_diagonal(thematrix))
